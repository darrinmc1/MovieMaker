...
CHARACTERS = {
    ...
    "Vex": {
        "base": (
            "Young woman in mid-20s, 5'6\" lithe acrobatic build, dusky olive complexion, sharp cheekbones, slightly upturned nose. "
            "HAIR — always exactly this: jet black hair, longer on the LEFT side falling to jaw length, "
            "RIGHT side of head shaved close to skin — a hard undercut. "
            ...
            "EYES — mismatched: LEFT eye is vivid bright green, RIGHT eye is burnished amber-gold. "
            "Both eyes must be visible and clearly different colours — this is her most distinctive feature. "
        ...
        "poses": [
            "full body portrait, standing facing viewer, weight on one hip, right hand resting on dagger hilt, "
            "head turned slightly so both mismatched eyes are visible — left green right gold — shaved right side of head clearly shown, dark background",
            "three-quarter view, crouching low on a stone ledge, cloak swept back, one dagger drawn, "
            "face turned toward viewer showing mismatched eyes, shaved undercut side visible, medieval rooftop background",
            "close-up bust portrait, sardonic half-smile, one eyebrow arched, "
            "both eyes filling frame — left vivid green right burnished gold — jet black hair with single silver streak, shaved right side, shadows framing face",
            "action pose, leaping between rooftops, cloak streaming behind, both daggers drawn, "
            "shaved undercut clearly visible from this angle, medieval fantasy city rooftops at twilight background",
        ]
    },
...
def generate_image(prompt: str, output_path: Path, reference_image_path: Path | None = None) -> bool:
    """
    Generate one image using flux/dev and save it.
    If reference_image_path is provided, uses image-to-image for consistency.
    """
    ...
    try:
        if reference_image_path and reference_image_path.exists():
            # consistent-character: locks face/hair while allowing completely new pose and background
            import base64
            ref_b64 = base64.b64encode(reference_image_path.read_bytes()).decode()
            ref_data_uri = f"data:image/png;base64,{ref_b64}"
            print(f"    Using consistent-character ref: {reference_image_path.name}")
            payload = {
                "prompt": prompt[:1500],
            ...
            # Text-to-image: initial reference generation
            payload = {
                "prompt": prompt[:1500],
        ...
        data = response.json()
        # consistent-character returns {"images": [...]} or {"image": {"url": ...}}
        if "images" in data:
...
def get_default_path(char_name: str) -> Path:
    """Returns path to the chosen default/canonical image for a character."""
...
def generate_for_character(char_name: str, overwrite: bool = False) -> None:
    ...
    if using_reference:
        ...
        print(f"  ℹ️  No default image yet — generating 4 initial options (text-to-image)")
    ...
    for i, pose in enumerate(char["poses"], start=1):
        ...
        # Use default image as reference if it exists (image-to-image for consistency)
