...
import { getNovelByTitle, createNovel, upsertChapter } from "@/app/actions/chapter-actions"
...
export function ChapterUploadForm({
  ...
  useEffect(() => {
    async function initializeNovel() {
      try {
        const { data: existingNovel, error: searchError } = await getNovelByTitle(novelTitle)
        ...
        if (existingNovel) {
          setDatabaseNovelId(existingNovel.id)
        } else {
          const { data: newNovel, error: insertError } = await createNovel({
            title: novelTitle,
        ...
        console.error("Failed to initialize novel:", error)
        toast({
  ...
  const handleSubmit = async (status: "draft" | "published") => {
    ...
    try {
      const { data, error, updated } = await upsertChapter({
      ...
      toast({
        ...
        description: `Chapter ${formData.chapterNumber} has been ${updated ? "updated" : "created"} and ${status === "published" ? "published" : "saved as draft"}.`,
      })
