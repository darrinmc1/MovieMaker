                        ...
                        <div key={i} className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                            ...
                            <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-2xl flex flex-col items-center justify-center relative overflow-hidden">
                                    ...
                                    {/* Pacing Timeline */}
                                    <div className="w-full h-8 bg-zinc-950 rounded-lg flex items-end gap-1 p-1 z-10">
                                        {[...Array(12)].map((_, idx) => (
                                            <div
                                                key={idx}
                                                className="flex-1 bg-blue-500/20 rounded-sm transition-all hover:bg-blue-500/40"
                                                style={{ height: `${20 + Math.random() * 60}%` }}
                                            ></div>
                                        ))}
                                    </div>
                                    <div className="text-[8px] text-zinc-600 font-black uppercase mt-1 z-10 w-full flex justify-between">
                                        <span>Start</span>
                                        <span>Pacing Momentum</span>
                                        <span>End</span>
                                    </div>
                            ...
                            {r.confidenceIndicators?.length > 0 && (
                                <section className="p-6 bg-zinc-950 border border-zinc-900 rounded-2xl">
                                    <h3 className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-4">Editorial Confidence Indicators</h3>
                                ...
                                <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {r.driftAlerts?.length > 0 && (
                                        <div className="bg-amber-950/20 border border-amber-900/30 p-6 rounded-2xl border-l-4 border-l-amber-600">
                                            <h3 className="text-[10px] font-black text-amber-500 uppercase tracking-widest mb-3">Drift Radar Alert</h3>
                            ...
                            {r.continuityErrors?.length > 0 && (
                                <section className="bg-red-950/20 border border-red-900/30 p-6 rounded-2xl">
                                    <div className="flex justify-between items-center mb-6">
                                        <h3 className="text-xs font-black text-red-500 uppercase tracking-widest">CRITICAL CONTINUITY</h3>
                            ...
                            {/* Inline Suggestions (Step 2 & Step 7 Diff) */}
                            <section className="space-y-6">
                                <h3 className="text-xs font-black text-zinc-500 uppercase tracking-widest px-1">PROPOSED REVISIONS</h3>
                                                ...
                                                <div className="flex gap-2">
                                                    <button
                                                        onClick={async () => {
                                                            ...
                                                            if (res.ok) alert("Character database updated!")
                                                        }}
                            ...
                            {/* New Characters Found (Phase 4) */}
                            {r.newCharactersFound?.length > 0 && (
                                <section className="space-y-4">
                                    <h3 className="text-xs font-black text-zinc-500 uppercase tracking-widest px-1 flex items-center gap-2">
                                        <span className="w-1.5 h-1.5 rounded-full bg-green-500"></span>
                                        New Narrative Entities Detected
                                    </h3>
                                    {r.newCharactersFound.map((nc: any, k: number) => (
                                        <div key={k} className="bg-zinc-950 border border-green-900/10 p-5 rounded-2xl relative group overflow-hidden">
                                            <div className="absolute top-0 right-0 p-4">
                                                <button
                                                    onClick={async () => {
                                                        const res = await fetch("/api/characters/update", {
                                                        ...
                                                        if (res.ok) alert(`${nc.name} added to Story Bible!`)
                                                    }}
                                                    className="px-3 py-1 bg-green-600 text-white text-[10px] font-black uppercase tracking-widest rounded-md hover:bg-green-500 transition-colors"
                                                >
                                                    Add to Story Bible
                                                </button>
                                            </div>
                                            <span className="text-[10px] font-black text-green-500 uppercase mb-1 block">Unregistered Entity</span>
                                            <p className="text-sm font-bold text-white mb-2">{nc.name}</p>
                                            <p className="text-xs text-zinc-400 font-medium mb-2">{nc.role}</p>
                                            <p className="text-[11px] text-zinc-500 italic leading-relaxed bg-black/40 p-3 rounded-lg border border-zinc-900/50">
                                                Observation: {nc.state}
                                            </p>
                                        </div>
                                    ))}
                                </section>
                            ...
                            {/* Cut Suggestions (Stage 3.1) */}
                            {r.cutSuggestions?.length > 0 && (
                                <section className="space-y-4">
                                    <h3 className="text-xs font-black text-zinc-500 uppercase tracking-widest px-1">STRUCTURAL CUT SUGGESTIONS</h3>
                                    {r.cutSuggestions.map((cs: any, k: number) => (
                                        <div key={k} className="bg-zinc-900 border border-zinc-800 p-6 rounded-2xl border-l-4 border-l-red-500">
                                            <div className="flex justify-between items-start mb-4">
                                                <div>
                                                    <span className="text-[10px] font-black text-red-500 uppercase mb-1 block">POTENTIAL CUT identified</span>
                                                    <p className="text-sm text-zinc-200 leading-relaxed font-medium">{cs.reason}</p>
                                                </div>
                                                <span className={`text-[10px] font-black px-2 py-0.5 rounded uppercase ${cs.impact === 'high' ? 'bg-red-500 text-white' : 'bg-zinc-800 text-zinc-400'}`}>
                                                    Impact: {cs.impact}
                                                </span>
                                            </div>
                                            <div className="flex gap-2 justify-end">
                                                <button
                                                    onClick={() => handleApproveCut(cs.id)}
                                                    className={`px-4 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg border transition-all ${approvedCuts.includes(cs.id) ? 'bg-red-500 text-white border-red-500' : 'bg-red-950/40 hover:bg-red-900/60 text-red-100 border-red-900/30'}`}
                                                >
                                                    {approvedCuts.includes(cs.id) ? 'Cut Approved' : 'Approve Cut'}
                                                </button>
                                                <button className="px-4 py-2 bg-zinc-800 hover:bg-zinc-700 text-zinc-400 text-[10px] font-black uppercase tracking-widest rounded-lg border border-zinc-700 transition-all">
                                                    Reject
                                                </button>
                                            </div>
                                        </div>
                            ...
                            {r.betaReactions?.length > 0 && (
                                <section className="space-y-4">
                                    <h3 className="text-xs font-black text-zinc-500 uppercase tracking-widest px-1">BETA READER REACTIONS (STIMULATION)</h3>
