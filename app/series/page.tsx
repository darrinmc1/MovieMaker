            ...
            <div className="max-w-6xl mx-auto">
                ...
                <section className="bg-zinc-950 border border-zinc-900 p-10 rounded-[3rem] shadow-inner mb-12">
                    ...
                    <div className="space-y-4">
                        {(actsData as any[]).filter(a => (a.continuity?.warnings?.length ?? 0) > 0).slice(0, 3).map((a, i) => (
                            <div key={i} className="p-6 bg-amber-950/10 border border-amber-900/20 rounded-2xl flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    <div className="w-10 h-10 bg-amber-900/20 rounded-full flex items-center justify-center text-amber-500 text-lg">📶</div>
                                    <p className="text-sm text-amber-100 font-medium">Risk in {a.heading}: {a.continuity.warnings[0].message}</p>
                                </div>
                                <span className="text-[10px] font-black bg-amber-900/40 text-amber-200 px-3 py-1 rounded-full uppercase">Continuity Risk</span>
                            </div>
                        ))}
                        {(actsData as any[]).filter(a => (a.promises?.length ?? 0) > 0).slice(0, 2).map((a, i) => (
                            <div key={i + 100} className="p-6 bg-blue-950/10 border border-blue-900/20 rounded-2xl flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    <div className="w-10 h-10 bg-blue-900/20 rounded-full flex items-center justify-center text-blue-500 text-lg">📌</div>
                                    <p className="text-sm text-blue-100 font-medium">Open Promise in {a.heading}: {a.promises[0].text}</p>
                                </div>
                                <span className="text-[10px] font-black bg-blue-900/40 text-blue-200 px-3 py-1 rounded-full uppercase">Narrative Debt</span>
                            </div>
                        ))}
                        {chapters.length === 0 && (
                            <p className="text-xs text-zinc-600 italic">Scan more acts to populate series-wide intelligence.</p>
                        )}
                    </div>
                </section>
